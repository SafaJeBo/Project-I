...
# Plotting
set size ratio 1/10
plot 'thermo_kin+pot.dat' u ($1*t_coef):($2*e_kb) t 'Ekin' with lp lt 2 ps 1, \
     'thermo_tot+msd.dat' u ($1*t_coef):($2*e_kb) t 'Etot' with lp lt 3 ps 1, \
     'thermo_kin+pot.dat' u ($1*t_coef):($3*e_kb) t 'ULJ' with points lt 1 ps 1
...
plot 'thermo_tot+msd.dat' u ($1*t_coef):($2*e_kb) t 'Etot' with lp lt 4 ps 0.5, \
...
plot 'thermo_temp+press.dat' u ($1*t_coef):($2*e_kb) t 'Tinst' with lp lt 5 ps 0.5, \
...
plot 'thermo_temp+press.dat' u ($1*t_coef):(kb*e_kb*$3/sig**3) t 'Pressure (Pa)' with lp lt 7 ps 0.5, \
...
plot 'thermo_temp+press.dat' u ($2*e_kb):($3*kb*e_kb/sig**3) t 'P=f(T)' with lp lt 7 ps 0.5, \
